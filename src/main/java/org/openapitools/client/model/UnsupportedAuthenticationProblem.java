...
public class UnsupportedAuthenticationProblem extends Problem {
  ...
  * @throws IOException if the JSON Object is invalid with respect to UnsupportedAuthenticationProblem
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (UnsupportedAuthenticationProblem.openapiRequiredFields.isEmpty()) {
          return;
        } else { // has reuqired fields
          throw new IllegalArgumentException(String.format("The required field(s) %s in UnsupportedAuthenticationProblem is not found in the empty JSON string", UnsupportedAuthenticationProblem.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!UnsupportedAuthenticationProblem.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UnsupportedAuthenticationProblem` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
