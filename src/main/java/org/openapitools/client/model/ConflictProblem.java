 ...
 */
@ApiModel(description = "You cannot create a new job if one is already in progress.")
...
public class ConflictProblem extends Problem {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ConflictProblem
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (ConflictProblem.openapiRequiredFields.isEmpty()) {
          return;
        } else { // has reuqired fields
          throw new IllegalArgumentException(String.format("The required field(s) `%s` is not found in the empty JSON string", ConflictProblem.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ConflictProblem.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ConflictProblem` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
